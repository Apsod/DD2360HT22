...
int main(int argc, char **argv) {
  ...
  checkCuda(cudaHostAlloc(&hostMemory,  (numelsA + numelsB + numelsC) * sizeof(DataType), cudaHostAllocDefault));
  hostA = &hostMemory[0];
  hostB = &hostMemory[numelsA];
  hostC = &hostMemory[numelsA + numelsB]; 
