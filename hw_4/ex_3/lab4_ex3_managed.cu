...
int main(int argc, char **argv) {
  ...
  int SHARED = TPB * sizeof *C;

  myprint("blocks: %d (%d * %d), tpb: %d\n", BLOCKS, blocks_per_inner, numelsC, TPB); 
  
  //@@ Insert code below to allocate Host memory for input and output
  checkCuda(cudaMallocManaged(&Memory,  (numelsA + numelsB + numelsC) * sizeof(DataType)));
  A = &Memory[0];
  B = &Memory[numelsA];
  C = &Memory[numelsA + numelsB]; 
  ...
  start_clock();
  gemm<<<BLOCKS, TPB, SHARED>>>(A, B, C, numARows, numAColumns, numBRows, numBColumns);
